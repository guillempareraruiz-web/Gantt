unit uOperariosCustomColEdit;

// ============================================================================
// Dialogo de alta/edicion de una definicion de columna custom de Operarios.
// Escribe en FS_PL_Cfg_GridColumns con GridId='OPERARIOS' e IsCustomField=1.
// ============================================================================
...
type
  TOperariosCustomColData = record
    ColumnKey: string;
    Caption: string;
  ...
  TfrmOperariosCustomColEdit = class(TForm)
    lblColumnKey: TLabel;
    lblCaption: TLabel;
    lblDataType: TLabel;
  ...
  private
    FIsNew: Boolean;
    FData: TOperariosCustomColData;
    function ValidColumnKey(const S: string): Boolean;
  public
    class function Execute(var AData: TOperariosCustomColData; AIsNew: Boolean): Boolean;
  end;
...
class function TfrmOperariosCustomColEdit.Execute(
  var AData: TOperariosCustomColData; AIsNew: Boolean): Boolean;
var
  F: TfrmOperariosCustomColEdit;
begin
  F := TfrmOperariosCustomColEdit.Create(Application);
  try
    F.FIsNew := AIsNew;
    F.FData := AData;
...
procedure TfrmOperariosCustomColEdit.FormShow(Sender: TObject);
begin
  if FIsNew then
    Caption := 'Nueva columna personalizada (Operarios)'
  else
    Caption := 'Editar columna personalizada (Operarios)';
...
function TfrmOperariosCustomColEdit.ValidColumnKey(const S: string): Boolean;
var
  I: Integer;
  Ch: Char;
begin
...
procedure TfrmOperariosCustomColEdit.btnAceptarClick(Sender: TObject);
var
  Key, Cap: string;
begin
