-- ===========================================================================
-- V051 - FS_PL_vw_Backlog / FS_PL_vw_BacklogPlanned: heredar del padre tambien
--        cuando el campo del hijo es CADENA VACIA, no solo NULL.
--
-- Problema: las operaciones (Nivel 3, OP) se guardaban con CodigoArticulo=''
-- (cadena vacia) en lugar de NULL. COALESCE solo salta NULL, asi que la vista
-- mostraba '' en vez de heredar el articulo de la OT padre. (La FechaCompromiso
-- si heredaba porque era NULL.)
--
-- Solucion: envolver los campos de TEXTO heredables en NULLIF(campo,'') dentro
-- del COALESCE, de modo que '' tambien caiga al padre/abuelo. Asi se corrigen
-- los datos YA existentes sin re-sincronizar. (El reader, en paralelo, ya pasa
-- a propagar articulo/fecha hacia las OP en nuevas sincronizaciones.)
--
-- Reescribe ambas vistas a partir de V049 (mismo contenido + NULLIF en strings).
-- ===========================================================================
